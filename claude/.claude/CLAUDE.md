...
Managed by `beautiful-aerc` project (`~/Projects/beautiful-aerc`). Stow package replaces the old `aerc` and `nvim-mail` dotfiles packages.

- **Config**: `~/.config/aerc/` (symlinked via `stow beautiful-aerc`)
- **Account**: `geoff@907.life` via Fastmail JMAP with caching enabled
- **Styleset**: `nord` (generated by theme system)
- **Filters**: `beautiful-aerc` Go binary (headers, html, plain subcommands)
