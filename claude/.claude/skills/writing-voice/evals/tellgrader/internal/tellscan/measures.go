...
// proseOnly blanks list-item lines, keeping newlines so line numbers still map to the
// input. The docs-register measures grade prose paragraphs only; a line is recognized as
// a list item the same way bulletRe and numberedListRe already recognize one for the
// scanner's other checks. See MEASURES.md for the selector this implements.
func proseOnly(s string) string {
	lines := strings.Split(s, "\n")
	for i, line := range lines {
		if bulletRe.MatchString(line) || numberedListRe.MatchString(line) {
...
// splitSentences, the same splitter and denominator CadenceCV uses, over the prose-only
// selector. See MEASURES.md for the full definition.
